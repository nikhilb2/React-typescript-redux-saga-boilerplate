...
function* requestLoadFoodPrefs(action) {
  const requestURL = `/get_food_pref.php?langid=en`
...
function* requestUpdateProfile(action) {
  ...
  if (action.newData.avatarUri) {
    ...
    const fileName =
      fileNamePos !== -1
        ...
        : null
    if (fileName) {
      formData.append(`fileToUpload[0]`, {
        uri: action.newData.avatarUri,
        type: 'image/*',
        name: fileName
      })
  ...
  const params = {}
  if (action.newData.firstname) {
    params.firstname = action.newData.firstname
  }
  if (action.newData.lastname) {
    params.lastname = action.newData.lastname
  }
  if (action.newData.country) {
    params.country = action.newData.country.id
  }
  if (action.newData.foodpref) {
    params.foodpref = action.newData.foodpref.id
  }
  if (imageUrl) {
